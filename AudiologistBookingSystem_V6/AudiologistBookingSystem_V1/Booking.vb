...
VALUES(" & aud.ReturnAudiologistID & ", " & pat.ReturnPatientID(conn) & ", " & appID & ", '" & roomUsed & "', '" & stringHandling.SQLDate(day) & "', '" & t1.ToString & "', '" & t2.ToString & "', " & interpreter & ")", conn)
    ...
    Public Sub BookAnnualLeave(ByVal conn As System.Data.Odbc.OdbcConnection)
        'choose start date
        'choose end date
        ...
        If startDate = endDate Then
            Console.WriteLine("Is your annual leave all day?")
        ...
        Console.Clear()
        Console.WriteLine("Are you booking a personal appointment: ")
        System.Threading.Thread.Sleep(1000)
        Select Case YesNo()
            Case True
...
VALUES(" & aud.ReturnAudiologistID & ", '" & stringHandling.SQLDate(startDate) & "', '" & startTime.ToString & "', '" & endTime.ToString & "', " & personalAppointment & ")")
            Try
                booked = True
...
")
    ...
    Public Function YesNo() As Boolean
        Console.CursorVisible = False
        Dim currentChoice As Integer = 1
        Dim choice As ConsoleKey
        Console.Clear()
