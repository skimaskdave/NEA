...
")
    ...
    Sub AddMeetingAttendants()
        ...
        Dim startTimes As New List(Of TimeSpan)
        Dim endTimes As New List(Of TimeSpan)
        Dim stringHandling As New ErrorHandling()
        Dim place As String
        ...
        While meetings.Count = 0
            ...
            place = stringHandling.TryString(1).ToUpper

            Dim rsFindMeetings As Odbc.OdbcDataReader
            Dim sqlFindMeetings As New Odbc.OdbcCommand("SELECT * FROM meeting WHERE place LIKE ?", Module1.GetConnection())
            sqlFindMeetings.Parameters.AddWithValue("place", "%" & place & "%")
            rsFindMeetings = sqlFindMeetings.ExecuteReader()

            Console.WriteLine("Meetings: " & place)
            While rsFindMeetings.Read
                meetings.Add(rsFindMeetings("meetingID"))
                startTimes.Add(rsFindMeetings("starttime"))
                endTimes.Add(rsFindMeetings("endtime"))
                days.Add(rsFindMeetings("date"))
                Console.WriteLine(meetings.Count & ". " & rsFindMeetings("description") & " " & rsFindMeetings("starttime").ToString & " - " & rsFindMeetings("endtime").ToString & " " & rsFindMeetings("date"))
            End While
        End While
        Dim meetingChoice As Integer = -1
        While meetingChoice < 0 Or meetingChoice > meetings.Count
        ...
        bookMeetAtt.BookMeetingAttendees(meetings(meetingChoice - 1), startTimes(meetingChoice - 1), endTimes(meetingChoice - 1), days(meetingChoice - 1))
        Console.WriteLine("Press any key to continue...")
        Console.ReadKey()
    End Sub
