Public Class Booking
    ...
    Public Sub ShowAllAppointments()
        ...
        Dim sqlShowAllApps As New Odbc.OdbcCommand("select * from patientbooking where date = '" & stringHandling.SQLDate(day) & "' order by starttime", Module1.GetConnection)
        rsShowAllApps = sqlShowAllApps.ExecuteReader
        While rsShowAllApps.Read
            Dim rsFindAudName As Odbc.OdbcDataReader
            Dim sqlFindAudName As New Odbc.OdbcCommand("select firstname, surname from audiologists where audiologistid = " & rsShowAllApps("audiologistid") & "", Module1.GetConnection)
    ...
    Public Sub BookPatient()
        ...
        If YesNo("Would you still like to book this patient appointment?") = True Then
            ...
            Dim sqlFreeRooms As New Odbc.OdbcCommand("SELECT room FROM patientbooking WHERE DATE = '" & stringHandling.SQLDate(day) & "' AND starttime = '" & t1.ToString & "'", Module1.GetConnection) 'find which rooms are free
...
VALUES(?, ?, ?, ?, ?, ?, ?, ?)", Module1.GetConnection)
    ...
    Public Sub BookPatient2()
        ...
        If YesNo("Would you still like to book this patient appointment?") = True Then
            ...
            Dim booked As Boolean = True
            Dim sqlBookTogether As New Odbc.OdbcCommand("INSERT INTO patientbooking(audiologistID, patientID, appointmentID, room, DATE, startTime, endTime, interpreter) 
VALUES(" & aud.ReturnAudiologistID & ", " & pat.ReturnPatientID() & ", " & appID & ", '" & roomUsed & "', '" & stringHandling.SQLDate(day) & "', '" & t1.ToString & "', '" & t2.ToString & "', " & interpreter & ")", Module1.GetConnection)
            Try
                sqlBookTogether.ExecuteNonQuery()
            Catch ex As Exception
                booked = False
...
VALUES(" & aud.ReturnAudiologistID & ", '" & stringHandling.SQLDate(startDate) & "', '" & startTime.ToString & "', '" & endTime.ToString & "', " & personalAppointment & ")", Module1.GetConnection)
            Try
                booked = True
...
VALUES(" & aud.ReturnAudiologistID & ", '" & stringHandling.SQLDate(startDate) & "', '" & startTime.ToString & "', '" & endTime.ToString & "', " & personalAppointment & ")", Module1.GetConnection)
    ...
    Public Sub BookRepairs()
        ...
        Dim sqlCheckReps As New Odbc.OdbcCommand("select count(*) from repairs where date = '" & stringHandling.SQLDate(day) & "'", Module1.GetConnection)
        ...
        If repsCovered = False Then
            ...
            aud.GetAudiologistInfo()
            Dim sqlBookRepairs As New Odbc.OdbcCommand("INSERT INTO repairs(audiologistid, DATE, starttime, endtime) VALUES (" & aud.ReturnAudiologistID & ", '" & stringHandling.SQLDate(day) & "', '" & startTime.ToString & "', '" & endTime.ToString & "')", Module1.GetConnection)
    ...
    Public Sub BookMeeting()
        ...
        Dim sqlBookMeeting As New Odbc.OdbcCommand("INSERT INTO meeting(place, DESCRIPTION, DATE, starttime, endtime) VALUES ('" & place & "', '" & desc & "', '" & stringHandling.SQLDate(day) & "', '" & startTime.ToString & "', '" & endTime.ToString & "')", Module1.GetConnection)
        ...
        Dim sqlMeetingID As New Odbc.OdbcCommand("select * from meeting where date = '" & stringHandling.SQLDate(day) & "' and starttime = '" & startTime.ToString & "' and endtime = '" & endTime.ToString & "' and description = '" & desc & "' and place = '" & place & "'", Module1.GetConnection)
    ...
    Public Sub BookMeetingAttendees(ByVal meetingID As Integer, ByVal startTime As TimeSpan, ByVal endTime As TimeSpan, ByVal day1 As Date)
        ...
        For i = 0 To meetingAuds.Count - 1
            Dim sqlBookAudIntoMeeting As New Odbc.OdbcCommand("insert into meetingattendants(meetingid, audiologistid) values (" & meetingID & ", " & meetingAuds(i).ReturnAudiologistID & ")", Module1.GetConnection)
...
AND DATE >= '" & stringHandling.SQLDate(startEndDates(0)) & "' AND DATE <= '" & stringHandling.SQLDate(startEndDates(1)) & "'", Module1.GetConnection)
...
AND DATE >= '" & stringHandling.SQLDate(startEndDates(0)) & "' AND DATE <= '" & stringHandling.SQLDate(startEndDates(1)) & "'", Module1.GetConnection)
    ...
    Public Function RandomAudSelection(ByVal startTime As TimeSpan, ByVal endTime As TimeSpan, ByVal dayOfWeek As String, ByVal reps As Boolean) As Audiologist
        Dim startEndDates As Date() = stringHandling.GetStartEndWeekDates(day)
        Dim rsAudiologistSelect As Odbc.OdbcDataReader 'select all audiologists avaliable at this time
...
WHERE firstname = '" & rsAudiologistSelect("firstname") & "' AND surname = '" & rsAudiologistSelect("surname") & "')", Module1.GetConnection)
                rsCheckMaxAppointments = sqlCheckMaxAppointments.ExecuteReader
                While rsCheckMaxAppointments.Read
...
")
    ...
    Public Function CheckAnnualLeaveCanHappen(ByVal startDate As Date, ByVal endDate As Date, ByVal startTime As TimeSpan, ByVal endTime As TimeSpan) As Boolean
        ...
        Dim sqlCheckAudsOff As New Odbc.OdbcCommand("SELECT DISTINCT count(audiologistid) FROM annualleave WHERE DATE >= '" & stringHandling.SQLDate(startDate) & "' AND DATE <= '" & stringHandling.SQLDate(endDate) & "'", Module1.GetConnection)
