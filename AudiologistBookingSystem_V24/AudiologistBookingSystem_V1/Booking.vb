...
VALUES(" & aud.ReturnAudiologistID & ", " & pat.ReturnPatientID() & ", " & appID & ", '" & roomUsed & "', '" & stringHandling.SQLDate(day) & "', '" & t1.ToString & "', '" & t2.ToString & "', " & interpreter & ")", Module1.GetConnection)
    ...
    Public Sub BookPatientUrgent()
        ...
        End If 'find out if the patient is a child
        Dim rsAppID As Odbc.OdbcDataReader
        Dim sqlAppID As New Odbc.OdbcCommand("select appointmentid from appointment where type = ? and child = ?") 'get appointment id (tells you length, type)
        sqlAppID.Parameters.AddWithValue("@type", appType)
        sqlAppID.Parameters.AddWithValue("@child", child)
        rsAppID = sqlAppID.ExecuteReader
        Dim appID As Integer
...
VALUES(" & aud.ReturnAudiologistID & ", '" & stringHandling.SQLDate(startDate) & "', '" & startTime.ToString & "', '" & endTime.ToString & "', " & personalAppointment & ")", Module1.GetConnection)
    ...
    Public Sub BookMeeting()
        ...
        Dim sqlMeetingID As New Odbc.OdbcCommand("select meetingid from meeting where date = '" & stringHandling.SQLDate(day) & "' and starttime = '" & startTime.ToString & "' and endtime = '" & endTime.ToString & "' and description = '" & desc & "' and place = '" & place & "'", Module1.GetConnection)
